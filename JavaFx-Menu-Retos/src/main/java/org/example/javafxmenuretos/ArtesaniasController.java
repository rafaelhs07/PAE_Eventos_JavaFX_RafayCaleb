...
public class ArtesaniasController {
    ...
    public void initialize() {
        ...
        actualizarTabla();
        lblEstado.setText("Catálogo de artesanías cargado.");
    }

    private void configurarColumnaImagen() {
        colImagen.setCellFactory(columna -> new TableCell<>() {

            private final ImageView imageView = new ImageView();

            @Override
            protected void updateItem(String nombreImagen, boolean vacio) {
                super.updateItem(nombreImagen, vacio);

                if (vacio || nombreImagen == null || nombreImagen.trim().isEmpty()) {
                    setGraphic(null);
                    setText(null);
                    return;
                }

                String ruta = nombreImagen.startsWith("/")
                        ? nombreImagen
                        : "/" + nombreImagen;
    ...
    private void nuevo() {
        limpiarCampos();
        lblEstado.setText("Formulario listo para registrar una nueva artesanía.");
    }
    ...
    private void guardar() {
        ...
        try {
            ...
            artesaniaDAO.agregarArtesania(artesania);

            actualizarTabla();
            limpiarCampos();

            lblEstado.setText(
                    "Artesanía guardada correctamente: " + nombre
            );
    ...
    private void buscar() {
        ...
        if (texto.isEmpty()) {
            actualizarTabla();
            lblEstado.setText("Mostrando todas las artesanías.");
            return;
        }
        ...
        for (Artesania artesania : artesaniaDAO.listarArtesanias()) {
            boolean coincideNombre = artesania.getNombre()
                    .toLowerCase()
                    .contains(texto);

            boolean coincideCategoria = artesania.getCategoria()
                    .toLowerCase()
                    .contains(texto);

            if (coincideNombre || coincideCategoria) {
                resultados.add(artesania);
            }
        }

        tablaArtesanias.setItems(resultados);

        lblCantidad.setText(
                "Artesanías encontradas: " + resultados.size()
        );

        lblEstado.setText(
                "Búsqueda realizada: " + resultados.size() + " resultado(s)."
        );
    }
    ...
    private void mostrarCatalogo() {
        actualizarTabla();
        lblEstado.setText("Mostrando el catálogo completo de artesanías.");
    }

    @FXML
    private void registrarVenta() {
        Artesania seleccionada = tablaArtesanias
                .getSelectionModel()
                .getSelectedItem();
        ...
        lblEstado.setText(
                "Venta registrada: " + seleccionada.getNombre()
                        + ". Stock restante: " + seleccionada.getStock()
        );

        mostrarAlerta(
                Alert.AlertType.INFORMATION,
                "Venta registrada",
                "Se vendió una unidad de: " + seleccionada.getNombre()
        );
    }
    ...
    private void actualizarTabla() {
        ...
        tablaArtesanias.setItems(lista);

        lblCantidad.setText(
                "Artesanías registradas: " + lista.size()
        );
    }
